...
#
#   progress update - Report worker progress
#   progress show   - Show current progress
#   progress clear  - Clear progress file
#
...
require_relative 'progress_manager'
...
class Orchestrator
  ...
  def run(args)
    ...
    when 'state'
      ...
      handle_state(args)
    when 'progress'
      @logger.info("Executing progress operation")
  ...
  def run_command(command, args)
    AgentAttribution.require_attribution!(context: "orch:#{command}")
    script_path = File.join(File.dirname(__FILE__), COMMANDS[command])
    @logger.debug("Dispatching to command script: #{script_path}", { args: args, agent_slug: AgentAttribution.agent_slug, agent_id: AgentAttribution.agent_id })
  ...
  def handle_progress(args)
    ...
    when 'update'
      phase = extract_option(args, '--phase')
      step = extract_option(args, '--step')
      detail = extract_option(args, '--detail')
      metrics_json = extract_option(args, '--metrics')
      ...
      metrics = metrics_json ? JSON.parse(metrics_json) : nil
      @logger.info("Updating progress", { phase: phase, step: step })
      result = @progress.update(phase: phase, step: step, detail: detail, metrics: metrics)
      puts result.to_yaml
    ...
    when 'show'
      ...
      puts @progress.current.to_yaml

    else
      puts "Unknown progress action: #{action}"
  ...
  def show_help
    puts <<~HELP
      ...
      PROGRESS COMMANDS:
        progress update  Report worker progress
                      --phase PHASE      Current phase (required)
                      --step STEP        Current step (required)
                      --detail TEXT       Human-readable detail
                      --metrics JSON      JSON metrics object

        progress show    Show current worker progress
        progress clear   Clear progress file (lead calls after phase)
