...
class StateManager
  ...
  # Coerces boolean-like strings ("true"/"false") to actual booleans
  # so YAML output uses unquoted true/false (not quoted 'true'/'false')
  def set(key, value)
    @state[key.to_s] = coerce_value(value)
    save_state
    value
  end
  ...
  def update(hash)
    hash.each { |k, v| @state[k.to_s] = coerce_value(v) }
    save_state
    @state
  end
  ...
  def set_bulk_mode(value)
    @state['bulk_mode'] = coerce_value(value)
    save_state
    value
  end
  ...
  # Coerce CLI string values to proper types for clean YAML serialization
  # CLI args are always strings, but YAML consumers expect proper types
  ...
  def load_state
    ...
    raw = YAML.safe_load(File.read(STATE_FILE)) || default_state
    # Coerce all values on load to handle state files written before
    # coerce_value was added (string 'true'/'false' → boolean)
    raw.transform_values { |v| coerce_value(v) }
