...
class FinalizeCommand
  ...
  def execute_commit_with_polling(result)
    ...
    loop do
      ...
      # Stage everything
      AgentAttribution.log_git_op('git add -A')
    ...
    # STEP 4: Execute commit (with persona attribution trailer)
    commit_subject = implementation_commit_subject
    full_commit_message = build_implementation_commit_message(commit_subject)

    @logger.info("Executing commit", { subject: commit_subject })
    commit_output = nil
    Tempfile.create('finalize_commit_msg') do |temp|
      temp.write(full_commit_message)
      temp.close
      commit_cmd = "git commit -F #{Shellwords.escape(temp.path)}"
      AgentAttribution.log_git_op(commit_cmd)
    ...
    @logger.info("Commit completed successfully", { attempts: attempt, sha: sha, subject: commit_subject })
    ...
    result['data']['executed_steps'] << "git commit -F <tempfile>  # message: #{commit_subject}"
    result['data']['commit_completed'] = true
    result['data']['commit_sha'] = sha
    result['data']['commit_subject'] = commit_subject
    result['data']['commit_message'] = full_commit_message
  ...
  def prepare_commit(result)
    # Metadata only — the commit was already created by execute_commit_with_polling.
    # No executable command strings are surfaced because the agent does not run
    # git for the implementation-complete commit.
    result['data']['commit'] = {
      'subject' => result['data']['commit_subject'] || implementation_commit_subject,
      'sha' => result['data']['commit_sha'],
      'note' => 'Committed directly by orchestrator with persona attribution trailer.'
    }
  end

  # Subject line for the implementation-complete commit.
  def implementation_commit_subject
    "feat(#{@change_name}): implementation complete"
  end

  # Full commit message body + AgentAttribution trailer for the
  # implementation-complete commit. Extracted so it can be unit-tested
  # without invoking the full polling-loop commit pipeline.
  def build_implementation_commit_message(subject = implementation_commit_subject)
    body = <<~MSG.strip
      #{subject}

      Auto-committed by PRD orchestration system at the close of the
      BUILD phase for change #{@change_name}.
    MSG
    body + AgentAttribution.commit_trailer
  end
  ...
  def handle_post_merge(result)
    ...
    AgentAttribution.log_git_op('git checkout development')
    ...
    AgentAttribution.log_git_op('git pull --rebase origin development')
