...
class PrebuildCommand
  ...
  def execute
    ...
    begin
      ...
      @logger.debug("Executing prebuild snapshot commit")
      execute_commit(result)
      return result if result['status'] == 'error'
  ...
  def prepare_commit(result)
    ...
    result['data']['commit'] = {
      ...
      'subject' => "chore(spec): #{@change_name} pre-build snapshot"
    }

    # Capture current branch for the agent's push step.
  ...
  # Direct execution: stage the openspec dir and create the snapshot commit
  # with persona attribution. Agent never runs git for this commit.
  def execute_commit(result)
    openspec_dir = "openspec/changes/#{@change_name}"

    stage_cmd = "git add #{Shellwords.escape(openspec_dir)}"
    AgentAttribution.log_git_op(stage_cmd)
    stage_output = `#{stage_cmd} 2>&1`
    unless $?.success?
      @logger.warn("Failed to stage OpenSpec files", { output: stage_output })
      result['status'] = 'error'
      result['errors'] << {
        'type' => 'git_stage_failed',
        'message' => "git add failed: #{stage_output.strip}",
    ...
    body = <<~MSG.strip
      ...
      Auto-committed by PRD orchestration system as the pre-build safety
      snapshot for change #{@change_name}.
    MSG
    full_message = body + AgentAttribution.commit_trailer

    Tempfile.create('prebuild_commit_msg') do |temp|
      temp.write(full_message)
      temp.close
      commit_cmd = "git commit -F #{Shellwords.escape(temp.path)}"
      AgentAttribution.log_git_op(commit_cmd)
      commit_output = `#{commit_cmd} 2>&1`
      unless $?.success?
        @logger.warn("Failed to commit prebuild snapshot", { output: commit_output })
        result['status'] = 'error'
        result['errors'] << {
          'type' => 'git_commit_failed',
          'message' => "git commit failed: #{commit_output.strip}",
          'resolution' => 'Investigate the git error and resolve manually before retrying.'
        }
        return
      end
    ...
    result['data']['commit']['sha'] = sha
    result['data']['commit']['committed'] = true
    result['data']['commit']['message'] = full_message
    @logger.info("Prebuild snapshot committed", { sha: sha, change_name: @change_name })
  end
    ...
    result['next_steps'] = [
      {
        ...
        'description' => "Pre-build snapshot committed by orchestrator (sha: #{result['data']['commit']['sha']})"
      },
      {
        'action' => 'push',
