...
description: Daily scan of the agentic economy landscape for sellable service opportunities,
  margin events, demand signals, and emerging infrastructure. Produces a daily report
  and accumulates findings into the Obsidian vault opportunity index.
argument-hint: '[--days N] [--deep] [--surface agents|human|both] [--reindex]'
allowed-tools: WebSearch, WebFetch, Read, Write, Bash(date *), Bash(mkdir -p *), Bash(wc
  -l *), Bash(tail *), Bash(head *)
metadata:
  short-description: Daily scan of the agentic economy landscape for sellable service
    opportunities, margin events, demand signals, and em...
---
