...
1. **Check if inputs are provided:**
   ...
   **Condition to check:** Is either `{{change_name}}` or `{{change_desc}}` empty, not provided, contains the literal text '{{change_name}}' or '{{change_desc}}' after variable substitution, or is undefined?
