...
**Run both commands now.** Parse the output and identify any overlaps. If no overlaps found, explicitly state 'No conflicts detected' in the report.
...
Target: 15-20 tasks
Warning: >20 tasks (consider splitting)
Blocking: >40 tasks (too large for single PRD)
```

**Severity:**
- ≤20 tasks → PASS
- 21-40 tasks → WARNING (recommend splitting)
- >40 tasks → ERROR (must split)
...
2. **[WARNING]** Scope Assessment
   Estimated 25 tasks exceeds target of 15-20. Consider splitting.
...
2. **[BLOCKING]** Scope Assessment
   Estimated 45 tasks is too large for a single PRD.
