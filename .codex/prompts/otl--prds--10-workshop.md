...
- **Scope-contained**: Target 15-20 OpenSpec tasks per PRD
...
**If the user answers 'n': Rework the synthesis by returning to the beginning of this phase. DO NOT proceed to the next phase until the user confirms 'y'.**
...
**Run this command now** to analyze git history for the subsystem:
...
- [FR1-3]: ~4 tasks (authentication/setup)
- [FR4-7]: ~6 tasks (core feature)
- [FR8-10]: ~4 tasks (integration)
...
**If estimated tasks ≤ 20:**
```
✓ Scope is appropriate (estimated [X] tasks, target 15-20)
...
**If estimated tasks > 20:**
```
⚠ Scope is too large (estimated [X] tasks, target 15-20)
...
**IMPORTANT:** After saving a new PRD (Create Mode only), **you MUST validate it before finishing. Wait for the validation command to complete and report its result (PASS/FAIL). Do not finish Phase 9 until validation returns.**
...
**Required:** Re-validate the PRD after fixes. If new issues are found, loop back. DO NOT exit until validation passes or the user explicitly chooses to exit.

Re-validate with `30: prd-validate {{prd_path}}`
```
