...
description: Extract and analyse YouTube video transcripts. Use when the user shares a YouTube URL or asks you to watch, review, summarise, or extract information from a YouTube video. Handles URL parsing, transcript fetching via the youtube-transcript-api Python library, and structured content extraction. Do NOT attempt to "watch" videos or scrape YouTube directly — this skill is the only reliable path.
allowed-tools: Bash(python3 *), Bash(python *), Bash(youtube_transcript_api *), Bash(pip install youtube-transcript-api), Read, Write
---
