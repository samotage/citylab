...
|---------|------|---------|
| 05 | `sprint-prompts` | Generate PRD workshop prompts for each sprint in a roadmap |
...
| 70 | `review-prd-set` | Adversarial cross-PRD review with Ralph loop — review, fix, re-review up to 3 passes |

## Quick Start

**Generate sprint prompts from a roadmap (do this first):**
```
05: sprint-prompts
```
...
**Review a batch of PRDs as a cohort (after all are individually validated):**
```
70: review-prd-set
```
...
## Cross-PRD Review (70: review-prd-set)

When PRDs are generated in batch for an epic, individual validation (`30: prd-validate`) checks each PRD in isolation. The cross-PRD review adds a cohort-level adversarial analysis that catches issues between PRDs, **then fixes them and re-reviews** — up to 3 passes (Ralph loop).

Evaluates 10 dimensions: scope overlap, boundary conflicts, dependency alignment, terminology consistency, contradictory requirements, gap detection, sequencing feasibility, scope creep signals, success criteria conflicts, shared resource contention.

**Pipeline position (validation is optional before this step):**

```
10: prd-workshop → [30: prd-validate] → 70: review-prd-set → 10: queue-add
```

**Prerequisites:** At least 2 pending PRDs. Individual validation is recommended but not required.

**Loop behaviour:**
- Pass 1: full adversarial review across all 10 dimensions
- Fix phase: edits PRDs to resolve critical and warning findings
- Passes 2-3: re-review for residual and newly introduced issues, fix again
- Clean exit: zero critical + zero warning findings (info accepted)
- Dirty exit: residual issues after 3 passes → checkpoint for human review

**Output:** A single consolidated report showing finding lifecycle across all passes, every edit made to PRDs, and dimension status per pass.
