...
**Run this command now.  Do not skip it.  Do not reuse results from a previous invocation — the filesystem may have changed.**
...
For each PRD found in the Step 1 scan output:
...
3. **Read validation status** by running:
   ```bash
   ruby orch/prd_validator.rb status --prd-path [prd-path]
   ```
4. **Read validation metadata** (if status is valid or invalid) by running:
   ```bash
