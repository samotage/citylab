...
4. **Investigate incomplete tasks**:
   ...
   - **Verification requires concrete evidence: a test passing, a file existing, or code matching the task description. 'Probably done' is not verified — mark as incomplete if evidence is ambiguous.**
...
1. **Run this command now — do not reuse cached diff output from earlier in the conversation.**

   From the repository root, use Claude Code's terminal to run:
