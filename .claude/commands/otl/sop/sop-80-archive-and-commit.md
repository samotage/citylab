...
4. **Step 0.4: Automatically validate task completion (CRITICAL):**
   ...
   - **If any tasks are incomplete:**
     - For each incomplete task:
       ...
       4. Determine if the task can be safely marked as complete based on evidence. **Verification requires concrete evidence: a test passing, a file existing, or code matching the task description. 'Probably done' is not verified — mark as incomplete if evidence is ambiguous.**
